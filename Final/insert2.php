...
if ($x && $connection->query($sql2) === TRUE && $connection->query($sql3) === TRUE && $connection->query($sql4) === TRUE) {
    ...
    $del1 = $connection->query("DELETE from car WHERE car_id = '$key'");
    $del2 = $connection->query("DELETE from car_details WHERE car_id = '$key'");
    $del3 = $connection->query("DELETE from car_status WHERE car_id = '$key'");
    $del4 = $connection->query("DELETE from security_details WHERE car_id = '$key'");
